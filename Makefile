...
run: test-tlb
	for stride in 4 16 64 256 1k 4k 16k ; do \
		echo -n "stride $$stride\n"; \
		echo 'size,ns,cycle' > outs/$$stride.csv; \
		for i in 4k 8k 16k 32k 64k 128k 256k 512k 1M 2M 4M 6M 8M 16M 32M 64M 128M 256M 512M ; do \
			./test-tlb -r $$i $$stride | tee -a outs/$$stride.csv; \
		done ; \
	done

	# for stride in 4 8 16 32 64 128 256 ; do \
#
